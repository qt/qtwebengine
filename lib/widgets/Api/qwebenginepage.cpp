...
bool QWebEnginePagePrivate::authenticationDialog(const QString &message, QString &username, QString &password)
{
    bool ret = false;

    if (this->authenDialog != 0) {
        this->authenDialog->setParent(0);
        delete this->authenDialog;
        this->authenDialog = 0;
    }
    this->authenDialog = new AuthenticationPopupDialog(message, view);
