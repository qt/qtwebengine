...
bool RenderWidgetHostViewQtDelegateWebPage::isVisible() const
{
    ...
    return false;
// FIXME: revive page visibbility API?
//    return m_page->visibilityState() == QWebEnginePage::VisibilityStateVisible;
}
...
{
    if (m_page)
        Q_EMIT m_page->contentsSizeChanged(QSize(width, height));
}
