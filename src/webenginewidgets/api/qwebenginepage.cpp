...
RenderWidgetHostViewQtDelegate *QWebEnginePagePrivate::CreateRenderWidgetHostViewQtDelegate(RenderWidgetHostViewQtDelegateClient *client, RenderingMode mode)
{
    Q_UNUSED(mode);
    Q_Q(QWebEnginePage);
    m_rwhvDelegate = new RenderWidgetHostViewQtDelegateWebPage(client);
    if (q)
        m_rwhvDelegate->setPage(q);
    return m_rwhvDelegate.data();
}
...
void QWebEnginePage::setViewportSize(const QSize &size) const
{
    Q_D(const QWebEnginePage);
    if (d->m_rwhvDelegate)
//        d->m_rwhvDelegate->resize(size.width(), size.height());
        d->m_rwhvDelegate->notifyResize();
    //FIXME: cache viewport size here for the getter
}
...
bool QWebEnginePage::hasFocus() const
{
    ...
    // FIXME: figure out where/how to store this
    return false;
}
