...
void RenderWidgetHostViewQt::handleInputMethodEvent(QInputMethodEvent *ev)
{
    ...
    if (ev->commitString().length())
        m_host->ImeConfirmComposition(ev->commitString().utf16(), gfx::Range::InvalidRange(), false);
    ...
    if (ev->preeditString().length()) {
        ...
        m_host->ImeSetComposition(ev->preeditString().utf16(), underlines, start, start + ev->replacementLength());
    }
}
