...
version_file_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..', 'src', 'chromium_version'))
...
def readVersionFile():
    ...
    if os.path.exists(version_file_path):
        with open(version_file_path, 'r') as version_file:
            chromium_version = json.loads(version_file.read().strip())
            print json.dumps(chromium_version, sort_keys=True, indent=2)
    else:
        print('Version file does not exist at expected path ' + version_file_path)
...
def readSubmodules():
    chromium_version = readVersionFile()
    if not chromium_version:
        return []

    version = chromium_version['version']
    print('\n... downloading and parsing DEPS for ' + chromium_version['channel'] + ' Chromium version ' + version + ' ...\n')

    response = urllib2.urlopen(base_deps_url + version + '/DEPS')
